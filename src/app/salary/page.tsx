...
export default function StaffSalaryPage() {
  ...
  return (
    <>
      <div className={`space-y-[var(--section-gap)] animate-in fade-in duration-150 pb-20 text-left transition-all duration-700 ease-in-out ${selectedSalaryId ? 'pr-[480px]' : ''}`} onClick={() => !selectedSalaryId && setSelectedSalaryId(null)}>
        ...
        <Card className="p-0 overflow-hidden border-[var(--border)] shadow-sm bg-white text-left">
          <div className="p-8 border-b border-[var(--border)] bg-[#F8F7F4]/50 flex items-center justify-between">
            <div>
              <h2 className="text-[16px] font-bold text-[var(--text)] font-serif uppercase tracking-tight leading-none mb-1.5">Payroll Detail Journal</h2>
                    ...
                    <td className="p-[12px_24px]">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col text-left">
                          <span className="text-[14px] font-bold text-[var(--text)] font-serif uppercase tracking-tight leading-none group-hover:text-[var(--gold)] transition-colors mb-1.5">{s.employeeName}</span>
      ...
      {/* Payslip Side Panel */}
      <div className={`fixed top-0 right-0 h-screen w-[480px] bg-white shadow-2xl z-[450] transform transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] border-l border-[var(--border)] flex flex-col ${selectedSalaryId ? 'translate-x-0' : 'translate-x-full'}`}>
        {selectedSalary ? (
          <div className="flex-1 flex flex-col p-10 overflow-y-auto scrollbar-hide text-left">
            <div className="flex items-center justify-between mb-12">
              <div className="flex items-center gap-5">
                <div className="w-12 h-12 bg-[var(--gold-lt)] rounded-2xl flex items-center justify-center text-[var(--gold)] border border-[var(--gold)]/20 shadow-sm transition-transform hover:rotate-12 duration-500">
                  <UserIcon className="w-6 h-6" />
                </div>
                <div className="text-left">
                  <h2 className="text-[18px] font-black text-[var(--text)] font-serif uppercase tracking-tight leading-none mb-2">{selectedSalary.employeeName}</h2>
                  <p className="text-[10px] text-[var(--text3)] font-black uppercase tracking-[3px] leading-none opacity-40 italic">{selectedSalary.role}</p>
                </div>
              </div>
              <Button 
                variant="secondary" size="icon"
                onClick={() => setSelectedSalaryId(null)}
                className="w-10 h-10 !p-0 flex items-center justify-center hover:rotate-90 transition-all duration-300 rounded-xl border-[var(--border)]"
              >
                ✕
              </Button>
            </div>
            ...
            <div className="mt-auto pt-8 border-t border-[var(--border)] sticky bottom-0 bg-white z-10 pb-4">
              {selectedSalary.status !== 'Paid' && (
                <Button v2={true} onClick={() => { updateSalaryStatus(selectedSalary.id, 'Paid'); showToast(`${selectedSalary.employeeName} authorized.`); }} className="w-full !py-5 shadow-2xl shadow-black/20 text-[11px] font-black uppercase tracking-[4px] bg-[#1e2a4a] hover:bg-black border-none animate-bounce">
          ...
          <div className="flex-1 flex flex-col items-center justify-center p-16 text-center opacity-20 grayscale">
              ...
              <p className="text-[12px] text-[var(--text3)] mt-6 leading-relaxed font-bold uppercase tracking-[2px] max-w-[280px]">Select a disbursement node from the ledger to visualize itemized identity metrics.</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-xl p-0 w-full max-w-xl shadow-2xl animate-in zoom-in-95 duration-300 border border-[var(--border)] overflow-hidden">
            <div className="p-8 border-b border-[var(--border)] bg-[var(--bg)] flex justify-between items-center text-left">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <h2 className="text-[20px] font-black text-[var(--text)] tracking-tight uppercase font-serif leading-none mb-1.5">Employment Onboarding</h2>
            ...
            <form onSubmit={handleAddStaff} className="p-8 space-y-8 text-left">
              ...
              <div className="pt-4 flex gap-5">
                <Button type="button" variant="secondary" v2={true} className="flex-1 h-12 rounded-xl border-[var(--border)] font-black uppercase tracking-[2.5px] text-[10px] bg-white shadow-sm" onClick={() => setIsAddStaffModalOpen(false)}>Abort Protocol</Button>
                <Button type="submit" v2={true} className="flex-[2] h-12 rounded-xl shadow-xl shadow-gold-500/20 font-black tracking-[4px] text-[10px]">Initialize Agent</Button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl p-0 w-full max-w-xl shadow-2xl animate-in zoom-in-95 duration-300 border border-[var(--border)] overflow-hidden">
            <div className="p-8 border-b border-[var(--border)] bg-[var(--bg)] flex justify-between items-center text-left">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <h2 className="text-[20px] font-black text-[var(--text)] tracking-tight uppercase font-serif leading-none mb-1.5">Cycle Authorization</h2>
            ...
            <div className="p-10 space-y-10 text-left">
              <div className="p-8 bg-[#F8F7F4] rounded-[32px] border border-dashed border-[var(--border)] relative overflow-hidden group">
                ...
                <p className="text-[15px] text-[var(--text2)] leading-relaxed font-bold uppercase tracking-tight relative z-10">
                  ...
                  the <span className="font-serif italic">{currentMonth}</span> cycle. This action marks all assets as <strong className="text-[var(--green)] font-black tracking-widest text-[13px] bg-white px-3 py-1 rounded-lg shadow-sm border border-[var(--green)]/10">VERIFIED</strong> in the ledger.
                </p>
              </div>
              <div className="flex gap-5">
                <Button v2={true} variant="secondary" className="flex-1 h-14 rounded-2xl border-[var(--border)] font-black uppercase tracking-[3px] text-[10px] bg-white shadow-sm" onClick={() => setIsBulkModalOpen(false)}>Abort Phase</Button>
                <Button v2={true} className="flex-[2] h-14 rounded-2xl shadow-xl shadow-green-500/20 bg-[var(--green)] hover:bg-[var(--green-dk)] border-none font-black uppercase tracking-[4px] text-[11px]" onClick={handleBulkAuthorize}>Authorize Cycle Nodes</Button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl p-0 w-full max-w-xl shadow-2xl animate-in zoom-in-95 duration-300 border border-[var(--border)] overflow-hidden">
            <div className="p-8 border-b border-[var(--border)] bg-[var(--bg)] flex justify-between items-center text-left">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  <h2 className="text-[20px] font-black text-[var(--text)] tracking-tight uppercase font-serif leading-none mb-1.5">Advance Protocol</h2>
            ...
            <form onSubmit={handleAddAdvance} className="p-8 space-y-8 text-left">
              <div className="space-y-4">
              ...
              <div className="pt-4 flex gap-5">
                <Button type="button" variant="secondary" v2={true} className="flex-1 h-12 rounded-xl border-[var(--border)] font-black uppercase tracking-[2.5px] text-[10px] bg-white shadow-sm" onClick={() => setIsAdvanceModalOpen(false)}>Abort</Button>
                <Button type="submit" v2={true} className="flex-[2] h-12 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-black tracking-[4px] text-[10px] shadow-xl shadow-amber-500/20">Execute Advance</Button>
              </div>
            </form>
          </div>
        </div>
      ...
      {isPreviewModalOpen && selectedSalary && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[600] flex items-center justify-center p-4 animate-in fade-in duration-500">
          <div className="bg-white rounded-xl p-0 w-full max-w-4xl shadow-2xl animate-in zoom-in-95 duration-300 border border-[var(--border)] overflow-hidden flex flex-col max-h-[95vh] text-left">
            <div className="p-8 border-b border-[var(--border)] bg-[#F8F7F4] flex justify-between items-center">
              <div className="flex items-center gap-5">
                ...
                <div className="text-left">
                  <h2 className="text-[20px] font-black text-[var(--text)] tracking-tight uppercase font-serif leading-none mb-2">Disbursement Document</h2>
            ...
            <div className="flex-1 bg-gray-100/30 p-12 overflow-y-auto scrollbar-hide">
            ...
            <div className="p-8 border-t border-[var(--border)] bg-white flex justify-between items-center text-left">
              <div className="flex items-center gap-4">
              ...
              <div className="flex gap-5">
                <Button variant="secondary" v2={true} className="h-12 px-8 rounded-xl border-[var(--border)] font-black uppercase tracking-[2.5px] text-[10px] bg-white shadow-sm" onClick={() => setIsPreviewModalOpen(false)}>Dismiss</Button>
                <Button v2={true} onClick={() => { window.print(); setIsPreviewModalOpen(false); }} className="h-12 px-10 bg-[#1e2a4a] text-white font-black uppercase tracking-[4px] rounded-xl shadow-2xl shadow-black/20 flex items-center gap-4 text-[11px] border-none">
